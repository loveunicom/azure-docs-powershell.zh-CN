...
Azure PowerShell 5.7.0 安装程序：[链接](https://github.com/Azure/azure-powershell/releases/download/v5.7.0-April2018/azure-powershell.5.7.0.msi)

ARM Cmdlet 的库模块：[链接](https://www.powershellgallery.com/packages/AzureRM/5.7.0)
...
## <a name="changes-since-last-release"></a>自上次发布以来的更改

#### <a name="general"></a>常规
* 已更新到最新版本的 Azure ClientRuntime

#### <a name="azurestorage"></a>Azure.存储
* 解决了已启用 FIPS 策略的计算机上发生上传 Blob 和上传文件 cmdlet 失败的问题
...
* 与认知服务管理 SDK 版本 4.0.0 集成。
* 添加了 Get-AzureRmCognitiveServicesAccountUsage 操作。

#### <a name="azurermcompute"></a>AzureRM.Compute
* `Get-AzureRmVmssDiskEncryptionStatus` 支持数据磁盘级别的加密状态
* `Get-AzureRmVmssVmDiskEncryptionStatus` 支持数据磁盘级别的加密状态
* 区域复原能力的更新
* “New-AzureRmVm”和“New-AzureRmVmss”（简单参数集）支持可用性区域。

#### <a name="azurermcontainerregistry"></a>AzureRM.ContainerRegistry
* 分解了对 Commands.Resources.Rest 和 ARM/存储 SDK 的依赖。

#### <a name="azurermdatalakestore"></a>AzureRM.DataLakeStore
* 添加了调试功能
* 已将 ADLS 数据平面 SDK 的版本更新为 1.1.2
* Export-AzureRmDataLakeStoreItem - 已弃用参数 PerFileThreadCount 和 ConcurrentFileCount，并引入了参数 Concurrency
* Import-AzureRMDataLakeStoreItem - 已弃用参数 PerFileThreadCount 和 ConcurrentFileCount，并引入了参数 Concurrency
* Get-AzureRMDataLakeStoreItemContent - 修复了大于 4MB 的内容的尾部行为
* Set-AzureRMDataLakeStoreItemExpiry - 引入了新参数集 SetRelativeExpiry 用于设置相对过期时间
* Remove-AzureRmDataLakeStoreItem - 已弃用参数 Clean。

#### <a name="azurermeventhub"></a>AzureRM.EventHub
* 修复了 New-AzureRmEventHubGeoDRConfiguration 中的 AlternameName

#### <a name="azurermkeyvault"></a>AzureRM.KeyVault
* 更新了 cmdlet 以包含管道场景
* 由于有重大更改的版本即将发布，添加了弃用消息

#### <a name="azurermnetwork"></a>AzureRM.Network
* 修复了 Network cmdlet 的错误消息

#### <a name="azurermservicebus"></a>AzureRM.ServiceBus
* 在 Rules 的 CorrelationFilter 中添加了“properties”，以支持 customproperties
* 更新了 New-AzureRmServiceBusGeoDRConfiguration 帮助，并修复了 Rules cmdlet 的输出
* 修复了 New-AzureRmServiceBusQueue 和 New-AzureRmServiceBusSubscription cmdlet 中的 auto-forward 属性
...
#### <a name="general"></a>常规
* 修复了 CloudShell 中的默认资源组的问题
* 修复了在模块导入过程中执行错误启动脚本的问题
...
#### <a name="azurermcompute"></a>AzureRM.Compute
* “New-AzureRmVM”和“New-AzureRmVMSS”支持数据磁盘。
* “New-AzureRmVM”和“New-AzureRmVMSS”支持按名称或按 ID 指定自定义映像。
* 日志分析功能
    - 添加了“Export-AzureRmLogAnalyticRequestRateByInterval”cmdlet
    - 添加了“Export-AzureRmLogAnalyticThrottledRequests”cmdlet

#### <a name="azurermcontainerinstance"></a>AzureRM.ContainerInstance
* 修复了容器注册表和 azure 文件卷装载的参数集问题

#### <a name="azurermdatafactoryv2"></a>AzureRM.DataFactoryV2
* 将 ADF.Net SDK 更新到了版本 0.6.0-preview，其中包含以下更改：
    - 添加了新的 AzureDatabricks LinkedService 和 DatabricksNotebook 活动
    - 在 HDInsightOnDemand LinkedService 中添加了 headNodeSize 和 dataNodeSize 属性
    - 为 SalesforceMarketingCloud 添加了 LinkedService、Dataset、CopySource
    - 在所有活动上添加了对 SecureOutput 的支持
    - 在 ForEach 活动上添加了新的 BatchCount 属性，以控制要运行的并发活动数量
    - 添加了新的 Filter 活动
    - 添加了链接服务参数支持

#### <a name="azurermdns"></a>AzureRM.Dns
* 对专用 DNS 区域的支持（公共预览版）
    - 添加了创建仅对关联虚拟网络可见的 DNS 区域的功能

#### <a name="azurermnetwork"></a>AzureRM.Network
* 更新模型类型以便与 DNS cmdlet 兼容。

#### <a name="azurermrecoveryservicessiterecovery"></a>AzureRM.RecoveryServices.SiteRecovery
* 对 ASR Azure 到 Azure Site Recovery 的迁移所做的更改（cmdlet 目前支持企业到企业、企业到 Azure、HyperV 到 Azure 以及 VMware 到 Azure 的操作）
...
* 在用于新建和设置存储帐户的 cmdlet（EnableEncryptionService 和 DisableEncryptionService）中废弃以下参数，因为静态加密在默认情况下处于启用状态，并且不能禁用。
    - New-AzureRmStorageAccount
    - Set-AzureRmStorageAccount

#### <a name="azurermwebsites"></a>AzureRM.Websites
* 修复了 Remove-AzureRmWebAppSlot 的帮助

## <a name="550---march-2018"></a>5.5.0 - 2018 年 3 月
#### <a name="azurermprofile"></a>AzureRM.Profile
* 修复了导入别名的问题
* 将 Newtonsoft.Json 的 10.0.3 版随 6.0.8 版一起加载
...
#### <a name="azurermanalysisservices"></a>AzureRM.AnalysisServices
* 修复了导入别名的问题
* 添加对防火墙和查询向外扩展功能的支持，以及对 2017-08-01 api 版本的支持。
...
#### <a name="azurermcompute"></a>AzureRM.Compute
* 'New-AzureRmVMSS' 以详细模式打印连接字符串。
* 'New-AzureRmVmss' 支持公共 IP 地址、负载均衡规则、入站 NAT 规则。
* WriteAccelerator 功能
    - 将 WriteAccelerator 开关参数添加到了以下 cmdlet：Set-AzureRmVMOSDisk Set-AzureRmVMDataDisk Add-AzureRmVMDataDisk Add-AzureRmVmssDataDisk
    - 将 OsDiskWriteAccelerator 开关参数添加到了以下 cmdlet：     Set-AzureRmVmssStorageProfile。
    - 将 OsDiskWriteAccelerator 布尔参数添加到了以下 cmdlet：     Update-AzureRmVM     Update-AzureRmVmss

#### <a name="azurermdatafactories"></a>AzureRM.DataFactories
* 修复了导致某些加密操作没有有意义错误的凭据加密问题
* 允许跨数据工厂共享集成运行时

#### <a name="azurermdatafactoryv2"></a>AzureRM.DataFactoryV2
* 为 "Set-AzureRmDataFactoryV2IntegrationRuntime" cmd 添加了参数 "SetupScriptContainerSasUri" 和 "Edition" 以启用自定义安装和版本选择功能
* 修复了导致某些加密操作没有有意义错误的凭据加密问题。
* 允许跨数据工厂共享集成运行时
...
#### <a name="azurermkeyvault"></a>AzureRM.KeyVault
* 修复了 Set-AzureRmKeyVaultAccessPolicy 的示例
...
#### <a name="azurermservicebus"></a>AzureRM.ServiceBus
* 为 Queue 添加了 EnableBatchedOperations 属性
* 为 Subscriptions 添加了 DeadLetteringOnFilterEvaluationExceptions 属性

#### <a name="azurermservicefabric"></a>AzureRM.ServiceFabric
* Service Fabric cmdlet 刷新
  - 更新了 ARM 模板
  - 失败的操作不再回滚
  - Add-AzureRmServiceFabricNodeType
    ...
    - 使用了现有 VMSS 子网
    - 所有操作都是幂等的
  - Remove-AzureRmServiceFabricNodeType 清除部分创建的 VMSS 和/或群集节点类型
  - 修复了复杂属性类型的 PSCluster 对象输出

#### <a name="azurermsql"></a>AzureRM.Sql
* 修复了导入别名的问题
* Get-AzureRmSqlServer、New-AzureRmSqlServer 和 Remove-AzureRmSqlServer 响应现在包括 FullyQualifiedDomainName 属性。

#### <a name="azurermwebsites"></a>AzureRM.Websites
* 修复了导入别名的问题
* New-AzureRMWebApp - 添加了参数集以用于简化的 WebApp 创建，并提供本地 Git 存储库支持。

## <a name="540---february-2018"></a>5.4.0 - 2018 年 2 月
#### <a name="azurermautomation"></a>AzureRM.Automation
* 从 New-AzureRmAutomationModule 到 Import-AzureRmAutomationModule 添加了别名

#### <a name="azurermcompute"></a>AzureRM.Compute
* 解决某些 Get cmdlet 的 ErrorAction 问题。

#### <a name="azurermcontainerinstance"></a>AzureRM.ContainerInstance
* 应用 Azure 容器实例 SDK 2018-02-01
    - 支持 DNS 名称标签

#### <a name="azurermdevtestlabs"></a>AzureRM.DevTestLabs
* 修复了以前无法正常运行的所有 GET cmdlet。

#### <a name="azurermeventhub"></a>AzureRM.EventHub
* 修复 Get-AzureRmEventHubGeoDRConfiguration 帮助中的 bug

#### <a name="azurermnetwork"></a>AzureRM.Network
* 添加了 cmdlet 以创建新的连接监视器
    - New-AzureRmNetworkWatcherConnectionMonitor
* 添加了 cmdlet 以更新连接监视器
    - Set-AzureRmNetworkWatcherConnectionMonitor
* 添加了 cmdlet 来获取连接监视器或连接监视器列表
...
* 更新了 Set-AzureRmApplicationGatewayBackendAddressPool 文档以删除弃用的示例
* 将 EnableHttp2 标志添加到了应用程序网关
    - 更新了 New-AzureRmApplicationGateway：添加了可选参数 -EnableHttp2
* 将 IpTags 添加到 PublicIpAddress
    - 更新了 New-AzureRmPublicIpAddress：添加了 IpTags
    - 用于添加 Iptag 的 New-AzureRmPublicIpTag
* 在 RouteTable 和 effectiveRoute 中添加 DisableBgpRoutePropagation 属性。

#### <a name="azurermresources"></a>AzureRM.Resources
* Register-AzureRmProviderFeature：添加了文档中缺少的示例
* Register-AzureRmResourceProvider：添加了文档中缺少的示例
...
* 添加了以下新命令以便执行异地灾难恢复操作。
  ...
  - 禁用灾难恢复，并停止从主命名空间向辅助命名空间复制更改
    - `Set-AzureRmEventHubGeoDRConfigurationBreakPair`
  - 调用灾难恢复故障转移并将别名重新配置为指向辅助命名空间
    - `Set-AzureRmEventHubGeoDRConfigurationFailOver`
  - 删除别名（灾难恢复配置）
    - `Remove-AzureRmEventHubGeoDRConfiguration`
* 添加了以下新命令，用于检查命名空间名称和 GeoDr 配置名称 - 别名可用性。
  - 检查命名空间名称或别名（灾难恢复配置）名称的可用性：
...
* 计算
  ...
  - 更新 Disable-AzureRmVmssDiskEncryption 以修复问题 https://github.com/Azure/azure-powershell/issues/5038
...
* Sql
  ...
  - 修复了问题 https://github.com/Azure/azure-powershell/issues/4974
    - 为审核 cmdlet 提供无效的 AUDIT_CHANGED_GROUP 值时，不再引发错误。此项将在即将发布的版本中删除。
  - 修复了问题 https://github.com/Azure/azure-powershell/issues/5046
...
* RecoveryServices.Backup
  ...
  * 修复 https://github.com/Azure/azure-powershell/issues/3164
