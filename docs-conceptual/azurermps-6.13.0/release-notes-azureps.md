...
* 将 ICMP 重新添加到受支持的 AzureFirewall 网络协议
...
* 已将可选参数 -KeyValue 添加到 New-AzureRmRelayKey cmdlet，该参数允许用户提供 KeyValue。

#### <a name="azurermresources"></a>AzureRM.Resources
* 更新 `New-AzureRmPolicyAssignment` 和 `Set-AzureRmPolicyAssignment` 中资源标识相关参数的帮助文档
...
* 为 Azure SQL 数据库托管实例和 Azure SQL 托管数据库上的 CRUD 操作添加了新的 cmdlet
...
* 允许在服务器或数据库上进行扩展的审核策略管理。
    - 添加了新的参数 (PredicateExpression)，用于筛选审核日志。
    - 修改了 Cmdlet，以便使用 SQL 客户端而不是旧客户端。
    - Set-AzureRmSqlServerAuditing。
    - Get-AzureRmSqlServerAuditing。
    - Set-AzureRmSqlDatabaseAuditing。
    - Get-AzureRmSqlDatabaseAuditing。
* 修复了将 Update-AzureRmSqlDatabaseVulnerabilityAssessmentSettings 与存储帐户名称参数集配合使用时出现的问题
